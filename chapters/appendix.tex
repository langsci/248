...
\begin{definition}  $s_{1^y} \subset u$ and $s_{1^y} \vDash \{ \soa{itc, {\cal A}, \sigma_{1}^{y}, {\cal B}} \}$, where 
...
%To define the product, start with two games $g_u(\omega_1) = \tuple{T_1,\subset \sep ACT_1,act_1 \sep N,\eta_1 \sep P_1 \sep \allowbreak H_1 \sep v_1}$ and $g_u(\omega_2) = \tuple{T_2,\subset \sep ACT_2,act_2 \sep N,\eta_2 \sep P_2 \sep H_2 \sep v_2}$ where $\omega_1$ and $\omega_2$ form a syntactic constituent like \textit{the waiter}.

%
%\subsection{Situations and Choices}

%Given an utterance situation $u$, the speaker $\cal A$ forms an intention to convey a (partial) content $\sigma_1$ (like the referential content of \textit{the}) and forms an intention to convey a (partial) content $\sigma_2$ (like the predicative content of \textit{waiter}) as part of a full utterance and chooses a possibly ambiguous locution $\omega_1$ (like \textit{the}) such that $\sigma_1 \in m_u(\omega_1)$ and a possibly ambiguous locution $\omega_2$ (like \textit{waiter}) such that $\sigma_2 \in m_u(\omega_2)$. 
