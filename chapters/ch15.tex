...
This is a partial information game as before but it is an \emph{illocutionary}  partial information game. $\cal A$ utters $\varphi = \textit{Smith weighs 150 lbs.}$ in $u$, its optimal locutionary meaning $\sigma^\ell \equiv \tau_0 = \emph{Smith weighs 150 lbs.}$ (and optimal parse $t$) are determined by $\cal B$ via the locutionary game $LG_u(\varphi)$, and then they trigger various illocutionary enrichments that need to be derived via corresponding illocutionary games.
