...
\end{equation}
~\\
$e(P^{\hbox{\emph{N}}},r_u)$ will provide the content of names like \textit{Hesperus} and \textit{Phosphorus}:\linebreak $e(P^{\textit{Hesperus}},r_u) = \hbox{Venus}$ and $e(P^{\textit{Phosphorus}},r'_u) = \hbox{Venus}$ where $r_u$, $r'_u$ are the two corresponding resource situations.\footnote{I am omitting certain complications involving conditioned infons that are considered in \citet{parikh:le} as the present definitions suffice for our purposes.}
...
If we consider the sentence ``Hesperus is Hesperus'' instead then its content would be: \[ \tau^{\ell} = \soa{R^{=};\ e(P^{\textit{Hesperus}},r_u);\ e(P^{\textit{Hesperus}},r_u)} \]
