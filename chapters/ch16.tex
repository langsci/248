...
Just to amplify my discussion, I will briefly analyze two more examples. The first is a slightly modified example from Grice~\cite{grice:lc}.\index{Grice, Paul} One agent asks another where Pierre lives, saying he wants to send him something, and the reply he gets is:

\begin{description}

\item Somewhere in the south of France. ($\eta$)
...
The next example is from Hugly and Sayward~\cite[page~22]{huglysayward:implicature}\index{Hugly, P.}\index{Sayward, C.} who argue convincingly that Grice's own way of handling it was circular. In a conversation about whether Eisenhower\index{Eisenhower, Dwight} was a great US president, one participant offers evidence of Eisenhower's generalship during the war and his great popularity, upon which $\cal A$ says:

\begin{description}

\item And he had a wonderful grin too. ($\eta'$)
...
Lastly, scalar implicatures of the kind discussed by Horn~\cite{horn:sploe, horn:qr},\index{Horn, Laurence} Levinson~\cite{levinson:pragmatics, levinson:pm},\index{Levinson, S. C.} and many others can also be handled similarly. The key thing to note here is that not all the implicatures that a scale may license will actually be generated because the relevant goals may not be present. It will all depend on the situation. In this sense, there are possibly no generalized implicatures of the kind Grice\index{Grice, Paul} had originally envisaged. Incidentally, Ross~\cite{ross:gip}\index{Ross, Ian} was probably the first to employ game-theoretic methods to analyze complex scalar implicatures. He showed that games of partial information (and Pareto-Nash equilibria) scale effortlessly toward this end.
