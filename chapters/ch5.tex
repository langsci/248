...
Although he never used the term, perhaps his account was \emph{abductive},\is{abduction} that is, it involved an inference to the best explanation, but choice is present even in abduction, and in all the examples he considered, a \emph{choice} among alternative explanations based on the agent's preferences never arose.\footnote{See C. S. \citet[page 151]{peirce:ai} for the first use of the term ``abduction.'' It is possible William \citet{whewell:pis} originated the idea. Here is a quote from \citet[pages 727--728]{hobbs:anlu}: ``In deduction, from $P$ and $P \Rightarrow Q$, we conclude $Q$. In induction, from $P$ and $Q$, or more likely a number of instances of $P$ and $Q$ together with other considerations, we conclude $P \Rightarrow Q$. Abduction is the third possibility. From an observable $Q$ and a general principle $P \Rightarrow Q$, we conclude that $P$ must be the underlying reason that $Q$ is true. We assume $P$ because it explains $Q$.
...
\lsptoprule
	& Logicism\is{logicism} & Wittgenstein\index{Wittgenstein, Ludwig} & Austin\index{Austin, J. L.} & Grice\index{Grice, Paul} & Lewis\index{Lewis, David}\\\midrule
...
As is well-known, Grice's framework consists of a general principle called the Cooperative Principle and four lower-level sets of conversational maxims that enable addressees to infer meanings beyond the literal content of an utterance. These are as follows \citep[pages~26--27]{grice:landc}:
...
\noindent To summarize the complex intention involved in meaning, the term ``M-intention'' was introduced. \citet[page~63]{schiffer:m} argued against both the sufficiency and necessity of these conditions and gave his own more adequate definition:
...
circularity. \citet[pages~194--195]{barwise:ssafa}  points out that such circular intentions involve circular (or non-well-founded) situations but does not spell out a definition. My account is as follows.
...
A related issue in \citet{devitt:mflp} \index{Devitt, Michael} is the use of the term ``metaphysics of meaning'' to describe speaker meaning and distinguish it from the ``epistemology of meaning'' to describe the addressee's (inferred) meaning. Speaker meaning and addressee meaning are certainly kept distinct in my framework. While Contextualists such as \citet{recanati:lm} \index{Recanati, Fran\c{c}ois} confuse them by discussing the addressee's interpretation in place of the speaker's production, the terms themselves are misleading and best avoided because the speaker has to take account of how the addressee will interpret his utterance and so the metaphysics and epistemology are intertwined and not separable. This interpenetration of the two will become completely obvious in Part~\textrm{III}.
...
%
%See Cohen'\citet[page~234]{cohen:ci} argument against Montague: 
%
%\begin{quote}
%
